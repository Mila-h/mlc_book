...
希腊的一组研究人员（Tsopani, Dallas, Tasika, & Tinto, 2012）则是在一群大学女子艺术体操新手身上证明了背景干扰效应。这项研究比较了传统的分段练习法和序列练习法对学习效果的影响。分段练习法（也是那所大学一贯采用的练习法）让体操运动员们在完成某个器械的全部练习之后才去练习下一个器械。而序列练习法则是让体操运动员们在练习课上按照按指定的顺序练习依次练习每种器械。练习共持续9周，每周两节90分钟的练习课。在这之后，实验人员对所有体操运动员进行了迁移测试，从技术性和艺术性两方面进行打分。实验结果表明，采用序列练习法的体操运动员比采用分段练习法的体操运动员表现得更好。上述的两项研究足以表明高背景干扰帮助初学者学习运动技能（如三种羽毛球发球方式以及几种艺术体操技能）。不过虽然这些研究都用的是现实中的运动技能，但毕竟还是在受控的实验情境下进行的。如果你希望看到更多关于背景干扰效应在现实世界的有效性的讨论，可以去阅读 Wrisberg 和 Liu （1991）。在这项研究中，学生们在一个实际的体育课的教学单元中学习羽毛球发球。最终他们在实际的课堂情境中得到的结果与 Goode 和 ­Magill 一致。
...
­第11章中的一些例子表明练习中的表现会让我们错误估计练习过程中的习得量。背景干扰效应则是这一情况的又一实例。实际上，练习中的表现不仅会导致对习得量的误判，而且还会影响学习者根据练习计划判断自己的习得量。判断自己习得量的过程被称为*元认知*，涉及到“对我们所了解的东西的了解”。评估这一信息的通常方法是要求人们预测自己在保留测试中的表现。在 Simon 和 Bjork（2001）的一项研究中，被试者要练习在数字键盘上以一定的顺序按下五个键。共有三种不同的按键顺序，每种顺序的目标用时也不同。研究表明，分段练习组的被试者总是会高估他们在第二天的测试中的表现。相比之下，随机练习组的练习者对测试成绩的估计则更加准确（见图16.5）。<font color='red'>[!! 本段参考原始文献进行了一定程度的改写]</font>
...
­­Guadagnoli 和 Lee（2004）对上述问题的观点与众不同却又引人深思。他们就任务特征与学习者特征对学习的影响提出了“挑战点假说”。这一假说可以通过任务难度和练习者的技能水平来确定最有效的练习情境。这里“挑战点”指的是一个对学习者来说具有挑战性的练习情境，可以恰到好处地提高技能学习的效果。为了确定挑战点，需要综合考量所学技能的难度和学习者的技能水平。假说里的“任务难度”分为两种：（1）“客观难度”，这指的是任务本身的难度，和任务本身的知觉要求和运动要求有关（比如第一章 Gentile 的技能分类法所考虑的那些）；（2）"主观难度"，这与学习者的技能水平有关（也就是说，有些任务新手觉得难，熟手则觉得没那么难）。该假说对背景干扰效应在练习安排中的效果做出了两个预测。首先，较高的背景干扰有助于学习难度最低的技能，但对于难度最高的技能来说就不那么适用了。其次，对于初学者或低技能水平的人来说，较低的背景干扰最有助于学习；而对更熟练的人来说，较高的背景干扰对他们将更有效。尽管这些预测符合作者建立的模型，但我们需要经验证据来确定它们在具体运动技能学习情境下的适用性。
...
在第13章中，你了解到学习的迁移量是练习和测验特征相似程度的函数，这意味着当练习的特征与测验的特征相同时，会出现最好的学习效果。然而，这个结论似乎与本章迄今为止的讨论不一致，在本章中，练习的可变性被认为是一个重要的练习特征，对未来考试成绩的成功至关重要。那么，这两个看似矛盾的原则怎么可能都是正确的呢？正如你在下面的章节中所看到的，我们已经讨论了练习的特殊性的各个方面，这些方面可以被看作是不同的，尽管与练习的可变性的好处有关。
...
实践的特殊性假说是人类学习的最古老的原则之一。它的起源可以追溯到20世纪初，当时桑代克（Thorndike, 1914; Thorndike & Woodworth, 1901）提出了相同要素理论，以解释为什么两种技能或技能学习情况之间会发生正向转移。简而言之，这个理论（在第13章中讨论过）提出，两种技能或情况的共同"元素"（即身体和心理特征）越多，学习或表现的转移量就越大。练习的特殊性假说也可以与我们在第三章中关于运动能力的讨论联系起来，在那里我们考虑了运动能力的特殊性假说，这是在20世纪60年代由富兰克林-亨利提出的（例如，亨利，1961a，1961b）。亨利的假说提出，运动能力是独立的和特定任务的，个人在许多运动能力方面有不同的水平。此外，一个人在两种不同技能上表现之间的关系将取决于技能之间共同能力的程度。最后，在第12章关于在学习技能的过程中会发生变化的各种表演者和表演特征的讨论中，你读到了练习的特殊性在技能学习中对一个与表演者有关的特征的影响，这个特征不会发生变化。
...
研究人员普遍认为，有足够的研究证据支持运动技能学习和表现的至少三个特征的练习特异性假说。我们在前几章中分别讨论了这些特征，但我们将在这里考虑它们，因为它们适用于练习的组织。在讨论了每个特征之后，我们将讨论练习特异性假说与练习可变性假说和情境干扰效应的关系问题。
...
正如你在第12章中所读到的，Proteau和他在加拿大的同事的研究（如Proteau，1992）表明，运动技能的学习是特定于练习过程中可用的感觉/知觉信息的来源。他们研究的具体重点是视觉和本体感觉的作用，即在执行一项技能时可获得的感觉/知觉信息。这种关注的动机强调视觉感官反馈在学习的早期阶段很重要，但随着练习的进行，其重要性会逐渐减弱，最终被本体感觉反馈对表现的重要性所取代（例如，Fleishman关于运动能力与技能学习关系的观点，这在第3章中讨论过；Schmidt模式理论假设技能学习过程中的变化，这在第12章讨论过）。
...
与提倡视觉反馈随着学习的继续而变得不那么重要的观点相反，Proteau的研究一直显示出相反的效果：如果一个人在练习时有视觉可用，那么在整个学习阶段，视觉仍然是感官信息的重要来源。事实上，保留或转移测试中，如果没有视觉成绩通常会下降。第12章中介绍了这一研究几个例子，其中研究人员对Proteau的假说在各种运动技能的学习中表示支持，如手动瞄准、力量举起和行走等。除了这些研究之外，Moradi, Movahedi和Salehi（2014）最近的一项研究发现，学习篮球罚球技能也有类似的视觉效应。
...
其中一些研究中特别值得注意的发现是，随着视觉练习量的增加，在随后的保持或转移测试中对视觉的需求也会增加（Proteau, Tremblay, & DeJaeger, 1998）。有证据表明，在保持和转移过程中对视觉需求的增加并不是因为在练习过程中视觉的可用性阻碍了本体感觉信息的处理，而是因为有视觉的练习鼓励人们宁愿使用相同的运动计划来完成没有视觉的技能(在转移测试中），就像他们在视觉练习中使用的那样（Mackrous & Proteau, 2007）。
...
对学习过程中视觉的实践特异性效应的另一种方法涉及使用观察性学习（Osman, Bird, & Heyes, 2005）。该实验涉及参与者在电脑键盘上学习一个连续反应时间任务中的八个项目序列。在观察了一个熟练的模特用右手演示这个序列后，参与者用他们的右手，然后用他们的左手练习这个任务。结果显示，对模特的观察只影响了右手的表现。
...
神经生理学证据也证明了感觉特征的练习特异性效应。例如，在一项研究中，研究人员使用fMRI观察与学习计时技能有关的大脑活动（Jantzen, Steinberg, & Kelso, 2005），参与者练习用食指敲击他们的拇指，以达到视觉或听觉节拍器的特定节奏。最终节拍器停止了，但参与者继续敲击节奏。在使用视觉节拍器练习时活跃的大脑区域在没有视觉节拍器的敲击过程中仍然活跃。这些脑区是视觉背流中的脑区，我们在第六章中讨论了视觉-行动视觉系统。
...
你在第10章中读到，编码特异性原则是记忆的一个公认的特征。这一原则指出，编码和检索环境与记忆表现之间存在着紧密的联系，即记忆测试（即检索）环境与练习（即编码）环境越相似，保留表现就会越好。回顾一下，表现背景问题的一个重要部分是有意记忆和偶然记忆之间的区别。有意的记忆是指当你必须记住环境背景的特定特征时，而偶然的记忆是指记住环境中相关但不重要的部分。例如，你正在练习网球中的回球，你被告知要估计并报告发球的球速。如果你被要求不仅要报告球速，还要报告球在发球区的落点，那么你很可能会报告这两点，尽管你没有被要求注意或记住球的落点。在这个例子中，报告球速代表有意的记忆；报告球的落点代表偶然的记忆。
...
大卫-赖特和他的同事的研究将编码特异性原则的性能背景方面扩展到运动技能学习上情况。他们的研究一致表明，实践环境背景中不属于要学习的实际技能的特征也会被学习（例如，Wright & Shea, 1991, 1994）。在这些实验中，参与者通常在电脑键盘上学习三位或四位数的按键序列。参与者看到电脑显示器上显示的数字键特定序列是他们需要学习的意向性项目。此外，每个序列都有颜色编码，这在数字序列和其颜色之间建立了关联。参与者没有被告知颜色编码的特征，这意味着颜色是表演背景的一个附带部分；它不是参与者被告知要学习的技能的一部分。然而，当他们同时看到数字序列（需要学习的有意项目）和相关的颜色编码（练习环境的附带部分）时，他们在转移测试中的表现比看到不同颜色的相同数字序列时更好。因此，他们不仅学会了被告知要学习的数字序列，而且还学会了与这些序列相关的颜色。有趣的是，在实验后的访谈中，参与者表示他们没有意识到数字与颜色的关系。
...
从调查表演研究中，人们对背景的学习比他们被明确指示的学习要多。技能的环境背景的一部分被纳入人们的学习内容。当这些环境的附带部分在测试中可用时，它们作为提示或辅助手段，帮助人们检索所学技能的记忆表征。相反，当这些偶然的环境背景特征在测试环境中不可用时，表现就会减弱。这种效应的例子包括舞蹈室和重量训练室中镜子的使用。因为镜子是练习环境的一部分，从镜子中得到的视觉反馈成为所学内容的一部分。当镜子不存在时，就像在舞台上的舞蹈表演或举重比赛中发生的那样，这些活动的表现会比有镜子存在时要差。
...
根据Shea和Wright（1995）的研究，决定在某种情况下执行哪种具体技能以及选择的速度取决于表演环境中有意和无意的特征之间的关系强度。技能训练的含义是在练习环境背景中尽可能多地包括测试环境背景的特征。例如，如果物理治疗中步态训练的目标是让病人能够在拥挤的商场中行走，作为他或她日常生活活动的一部分，那么当治疗师确定病人可以从事这种类型的活动时，治疗课程应该包括在拥挤的商场中的监督下行走。就其在体育情况下的应用而言，在练习和比赛期间对球员环境背景的偶然部分的学习导致了在体育比赛中普遍观察到的 "主场优势"。这意味着 "主场 "的表现环境的部分对主队来说更熟悉，因为他们在该环境下练习时学习了这些部分。因此，他们已经学会了有助于他们表现的重要情境线索，尽管他们从未刻意练习过学习这些线索。
...
在第13章中，对积极转移的解释之一是考虑到认知的相似性。技能或学习情况所要求的过程。根据适合转移的加工理论，当一个人在学习一项技能时，最好的练习类型需要与转移测试中所需要的认知加工活动类型相同，而不考虑练习和测试技能和情况之间的物理相似性。例如，如果测试情况需要快速决策，那么练习应该包括需要快速决策活动。这种快速决策情况的一个很好的例子是，当一个足球运动员在场上运球，看到一个后卫很快就会与他或她对峙，球员必须非常迅速地决定是继续运球，还是将球传给队友。造成这种关系的原因之一是，作为练习的一部分的认知处理成为所学内容的一部分。因此，练习情况下的认知处理要求是一种练习
...
Pellecchia(2005)的一个实验提供了一个很好的例子，说明技能学习过程中认知需求的实践特性。参与者在一个平衡平台上练习静止30秒，平台表面覆盖有泡沫垫，以减少体感反馈，增加对前庭和视觉反馈的依赖。两组参与者只练习平衡技能（单任务训练），或将该技能与认知辅助任务（双任务训练）结合起来，即从每个试验中随机出现的三位数中倒数。结果显示，当两组在一周后进行双任务条件下的转移测试时，单任务训练组的姿势摇摆有所增加，而双任务训练组则没有，这表明双任务训练提出的认知处理要求成为所学内容的一部分。练习期间的认知处理要求与测试之间的相似性导致了一周后在双任务转移测试中的成功表现。
...
Song和Bdard（2015）最近的一项研究显示，在双任务练习中，环境特异性效应特别强。当参与者同时练习瞄准任务和辅助辨别任务时，当后来用辅助任务测试时，他们保留了瞄准动作的准确性；但当他们在没有辅助任务的情况下被测试时，他们的表现就会退步到基线水平，尽管在没有辅助任务的情况下会有额外的注意能力。
